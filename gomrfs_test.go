...
// This test should be run with the race detector on:
// go test -run TestGormFsDataRace -race
func TestGormFsDataRace(t *testing.T) {
	const dir = "test_dir"
	fs := TestingFs(t)
	...
	go func() {
		...
		for i := 0; i < n; i++ {
			...
			if err := afero.WriteFile(fs, fname, []byte(""), 0777); err != nil {
				panic(err)
			}
...
// root is a directory
func TestGormFsRootDirMode(t *testing.T) {
	t.Parallel()
...
func TestGormFsMkdirAllMode(t *testing.T) {
	t.Parallel()
...
func TestGormFsMkdirAllNoClobber(t *testing.T) {
	t.Parallel()
...
func TestGormFsDirMode(t *testing.T) {
	fs := TestingFs(t)
...
func TestGormFsUnexpectedEOF(t *testing.T) {
	t.Parallel()

	fs := TestingFs(t)

	if err := afero.WriteFile(fs, "file.txt", []byte("abc"), 0777); err != nil {
		t.Fatal(err)
	}
...
func TestGormFsChmod(t *testing.T) {
	t.Parallel()
...
/*
func TestGormFsMkdirModeIllegal(t *testing.T) {
	t.Parallel()
...
/*
func TestGormFsOpenFileModeIllegal(t *testing.T) {
	t.Parallel()
...
// support symlinks.
/*
func TestGormFsLstatIfPossible(t *testing.T) {
	t.Parallel()

	fs := afero.Fs(TestingFs(t))

	// We assert that fs implements Lstater
	fsAsserted, ok := fs.(afero.Lstater)
	if !ok {
